...
class DashboardApp {
  constructor() {
    this.apiKey =
      localStorage.getItem("fb_crawl_api_key") ||
      "dev-api-key-1234567890123456789012";
    this.baseUrl = window.location.origin;
    this.currentView = "dashboard";
    this.refreshInterval = null;
    this.installationId =
      localStorage.getItem("lead_finder_installation_id") ||
      crypto.randomUUID();
  ...
  initElements() {
    ...
    this.metricActiveSessions = document.getElementById(
    ...
    this.tableAdminLicenseKeys = document.getElementById(
      "table-admin-license-keys-body",
    );
    this.tableAdminAccounts = document.getElementById(
      "table-admin-accounts-body",
    );
    this.tableAdminSubscriptions = document.getElementById(
      "table-admin-subscriptions-body",
    );
    this.tableAdminDevices = document.getElementById(
      "table-admin-devices-body",
    );
    this.tableAdminAuditEvents = document.getElementById(
    ...
    this.modalGeneratedLicense = document.getElementById(
      "modal-generated-license",
    );
    this.modalAdminSubscriptions = document.getElementById(
    ...
    this.productAdminLoginPanel = document.getElementById(
      "product-admin-login-panel",
    );
    this.productAdminWorkspace = document.getElementById(
      "product-admin-workspace",
    );
    this.productAdminSessionStatus = document.getElementById(
      "product-admin-session-status",
    );
    ...
    this.btnOpenImportSession = document.getElementById(
      "btn-open-import-session",
    );
    this.btnOpenExtractSession = document.getElementById(
      "btn-open-extract-session",
    );
    ...
    this.btnPreviewSyncScans = document.getElementById(
      "btn-preview-sync-scans",
    );
    ...
    this.adminLicensesPagination = {
      pageIndex: 0,
      cursorHistory: [null],
      nextCursor: null,
    };
    this.adminAccountsPagination = {
      pageIndex: 0,
      cursorHistory: [null],
      nextCursor: null,
    };
    this.adminAuditPagination = {
      pageIndex: 0,
      cursorHistory: [null],
      nextCursor: null,
    };
  ...
  bindEvents() {
    ...
    const btnSaveSettings = document.getElementById(
      "btn-save-api-key-settings",
    );
    if (btnSaveSettings)
      btnSaveSettings.addEventListener("click", () => this.saveSettings());

    if (this.formProductLogin) {
      this.formProductLogin.addEventListener("submit", (event) =>
        this.handleProductLogin(event),
      );
    }
    if (this.btnProductLogout) {
      this.btnProductLogout.addEventListener("click", () =>
        this.handleProductLogout(),
      );
    }
    if (this.formCreateLicense) {
      this.formCreateLicense.addEventListener("submit", (event) =>
    ...
    if (unlimitedDuration) {
      unlimitedDuration.addEventListener("change", () =>
        this.toggleUnlimitedDuration(),
      );
    }
    const unlimitedContacts = document.getElementById(
      "unlimited-contact-limit",
    );
    if (unlimitedContacts) {
      unlimitedContacts.addEventListener("change", () =>
        this.toggleUnlimitedContacts(),
      );
    }
    const unlimitedDevices = document.getElementById("unlimited-max-devices");
    if (unlimitedDevices) {
      unlimitedDevices.addEventListener("change", () =>
        this.toggleUnlimitedDevices(),
      );
    }
    const refreshLicenses = document.getElementById(
      "btn-refresh-admin-licenses",
    );
    if (refreshLicenses)
      refreshLicenses.addEventListener("click", () =>
        this.loadAdminLicenses(true),
      );
    const refreshAccounts = document.getElementById(
      "btn-refresh-admin-accounts",
    );
    if (refreshAccounts)
    ...
    const adminLicensesPrevious = document.getElementById(
      "btn-admin-licenses-previous",
    );
    if (adminLicensesPrevious)
      adminLicensesPrevious.addEventListener("click", () =>
        this.changeAdminPage("licenses", -1),
      );
    const adminLicensesNext = document.getElementById(
      "btn-admin-licenses-next",
    );
    if (adminLicensesNext)
      adminLicensesNext.addEventListener("click", () =>
        this.changeAdminPage("licenses", 1),
      );
    const adminAccountsPrevious = document.getElementById(
      "btn-admin-accounts-previous",
    );
    if (adminAccountsPrevious)
      adminAccountsPrevious.addEventListener("click", () =>
        this.changeAdminPage("accounts", -1),
      );
    const adminAccountsNext = document.getElementById(
      "btn-admin-accounts-next",
    );
    if (adminAccountsNext)
      adminAccountsNext.addEventListener("click", () =>
        this.changeAdminPage("accounts", 1),
      );
    const adminAuditPrevious = document.getElementById(
      "btn-admin-audit-previous",
    );
    if (adminAuditPrevious)
      adminAuditPrevious.addEventListener("click", () =>
        this.changeAdminPage("audit", -1),
      );
    ...
    // Modal open buttons
    if (this.btnOpenCreateJob)
      this.btnOpenCreateJob.addEventListener("click", () =>
        this.openModal(this.modalCreateJob),
      );
    if (this.btnOpenImportSession)
      this.btnOpenImportSession.addEventListener("click", () =>
        this.openModal(this.modalImportSession),
      );
    if (this.btnOpenExtractSession)
      this.btnOpenExtractSession.addEventListener("click", () =>
        this.openModal(this.modalExtractSession),
      );
    if (this.btnOpenAddProxies)
      this.btnOpenAddProxies.addEventListener("click", () =>
        this.openModal(this.modalAddProxies),
      );
    ...
    // Form Submissions
    if (this.formCreateJob)
      this.formCreateJob.addEventListener("submit", (e) =>
        this.handleCreateJob(e),
      );
    if (this.formFilterLeads)
      this.formFilterLeads.addEventListener("submit", (e) =>
        this.handleFilterLeads(e),
      );
    if (this.formImportSession)
      this.formImportSession.addEventListener("submit", (e) =>
        this.handleImportSession(e),
      );
    if (this.formExtractSession)
      this.formExtractSession.addEventListener("submit", (e) =>
        this.handleExtractSession(e),
      );
    if (this.formAddProxies)
      this.formAddProxies.addEventListener("submit", (e) =>
        this.handleAddProxies(e),
      );
    ...
    if (this.formEditSession)
      this.formEditSession.addEventListener("submit", (e) =>
        this.handleEditSessionSubmit(e),
      );
    if (this.formEditProxy)
      this.formEditProxy.addEventListener("submit", (e) =>
        this.handleEditProxySubmit(e),
      );
    ...
    if (btnJobsPrev) {
      btnJobsPrev.addEventListener("click", () => {
        if (this.jobsPagination.pageIndex > 0) {
          ...
          this.loadJobsData(
            this.jobsPagination.cursorHistory[this.jobsPagination.pageIndex],
          );
        }
      });
    }
    ...
    if (btnJobsNext) {
      btnJobsNext.addEventListener("click", () => {
        if (this.jobsPagination.nextCursor) {
          this.jobsPagination.pageIndex++;
          this.jobsPagination.cursorHistory[this.jobsPagination.pageIndex] =
            this.jobsPagination.nextCursor;
    ...
    if (btnLeadsPrev) {
      btnLeadsPrev.addEventListener("click", () => {
        if (this.leadsPagination.pageIndex > 0) {
          ...
          this.loadLeadsData(
            this.leadsPagination.cursorHistory[this.leadsPagination.pageIndex],
          );
        }
      });
    }
    ...
    if (btnLeadsNext) {
      btnLeadsNext.addEventListener("click", () => {
        if (this.leadsPagination.nextCursor) {
          this.leadsPagination.pageIndex++;
          this.leadsPagination.cursorHistory[this.leadsPagination.pageIndex] =
            this.leadsPagination.nextCursor;
    ...
    if (btnSessionsNext) {
      btnSessionsNext.addEventListener("click", () => {
        const maxPages = Math.ceil(
          this.sessionsPagination.allItems.length /
            this.sessionsPagination.limit,
        );
    ...
    if (btnProxiesNext) {
      btnProxiesNext.addEventListener("click", () => {
        const maxPages = Math.ceil(
          this.proxiesPagination.allItems.length / this.proxiesPagination.limit,
        );
    ...
    if (formSettings)
      formSettings.addEventListener("submit", (e) =>
        this.handleSaveFBNumberSettings(e),
      );

    const formWorkerSettings = document.getElementById(
      "form-update-worker-settings",
    );
    if (formWorkerSettings)
      formWorkerSettings.addEventListener("submit", (e) =>
        this.handleSaveWorkerSettings(e),
      );

    const btnResetCooldown = document.getElementById("btn-reset-cooldown");
    if (btnResetCooldown)
      btnResetCooldown.addEventListener("click", () =>
        this.handleResetCooldown(),
      );

    const btnToggleToken = document.getElementById(
    ...
    if (btnTestToken)
      btnTestToken.addEventListener("click", () =>
        this.handleTestFBNumberToken(),
      );

    // Action Type Selector (Dynamic Form Options & Placeholders)
    const jobActionSelect = document.getElementById("job-action");
    if (jobActionSelect) {
      jobActionSelect.addEventListener("change", (e) =>
        this.handleJobActionChange(e.target.value),
      );
    ...
    if (enrichCheckbox && subOptionsEnrich) {
      enrichCheckbox.addEventListener("change", () => {
        subOptionsEnrich.style.display = enrichCheckbox.checked
          ? "block"
          : "none";
      });
    }
    ...
    if (this.btnExportCsv)
      this.btnExportCsv.addEventListener("click", () =>
        this.exportLeads("csv"),
      );
    if (this.btnExportJson)
      this.btnExportJson.addEventListener("click", () =>
        this.exportLeads("json"),
      );

    // FBNumber Scans Sync Triggers
    if (this.btnOpenSyncScans) {
      this.btnOpenSyncScans.addEventListener("click", () =>
        this.handleOpenSyncScansModal(),
      );
    }
    ...
    if (this.btnPreviewSyncScans) {
      this.btnPreviewSyncScans.addEventListener("click", () =>
        this.handleExecuteSyncScans(true),
      );
    }
  ...
  handleJobActionChange(action) {
    ...
    const autoBatchOptions = document.getElementById(
      "group-auto-batch-options",
    );
    const enrichCheckbox = document.getElementById("job-opt-enrich-profiles");
    const subOptionsEnrich = document.getElementById("sub-options-enrich");
    ...
    if (!targets) return;
    if (autoBatchOptions)
      autoBatchOptions.style.display = action === "members" ? "block" : "none";

    if (action === "members") {
      targets.placeholder =
        "https://www.facebook.com/groups/782850425639223\nhttps://www.facebook.com/groups/example_group/members";
      if (targetsHint)
        targetsHint.innerHTML =
          "💡 <strong>Mục tiêu:</strong> Nhập đường link Nhóm (Group URL) hoặc link trang Thành viên Nhóm.";
      if (groupDepth) groupDepth.style.display = "none";
      if (rowSafetyParams)
        rowSafetyParams.style.gridTemplateColumns = "1fr 1fr";
      if (rowUsersSteps) rowUsersSteps.style.display = "grid";
    } else if (action === "comments") {
      targets.placeholder =
        "https://www.facebook.com/permalink.php?story_fbid=123456789&id=1000000\nhttps://www.facebook.com/page_name/posts/123456789";
      if (targetsHint)
        targetsHint.innerHTML =
          "💡 <strong>Mục tiêu:</strong> Nhập đường link bài viết hoặc video / Reels cần quét danh sách bình luận.";
      if (groupDepth) groupDepth.style.display = "none";
      if (rowSafetyParams)
        rowSafetyParams.style.gridTemplateColumns = "1fr 1fr";
      if (rowUsersSteps) rowUsersSteps.style.display = "grid";
      ...
      if (targetsHint)
        targetsHint.innerHTML =
          "💡 <strong>Mục tiêu:</strong> Nhập đường link bài viết cần quét danh sách người thả cảm xúc (Like, Tim, Haha...).";
      if (groupDepth) groupDepth.style.display = "none";
      if (rowSafetyParams)
        rowSafetyParams.style.gridTemplateColumns = "1fr 1fr";
      if (rowUsersSteps) rowUsersSteps.style.display = "grid";
      ...
      if (targetsHint)
        targetsHint.innerHTML =
          "💡 <strong>Mục tiêu:</strong> Nhập đường link trang cá nhân (Profile URL) để quét danh sách bạn bè / người theo dõi.";
      if (groupDepth) groupDepth.style.display = "block";
      if (rowSafetyParams)
        rowSafetyParams.style.gridTemplateColumns = "1fr 1fr 1fr";
      if (rowUsersSteps) rowUsersSteps.style.display = "grid";
      ...
      if (targetsHint)
        targetsHint.innerHTML =
          "💡 <strong>Mục tiêu:</strong> Nhập đường link trang cá nhân (Profile URL) để bóc tách thông tin hồ sơ chi tiết.";
      if (groupDepth) groupDepth.style.display = "none";
      if (rowSafetyParams)
        rowSafetyParams.style.gridTemplateColumns = "1fr 1fr";
  ...
  switchView(viewName) {
    if (viewName !== "product-admin") {
      this.clearGeneratedLicenseKey();
      this.clearRevealedLicenseKeys();
    }
  ...
  async fetchApi(endpoint, options = {}) {
    ...
    try {
      ...
      if (response.status === 401 || response.status === 403) {
        ...
        if (throwOnError)
          throw new Error("Sai API Key hoặc không có quyền truy cập.");
        return null;
      }
      if (!response.ok) {
  ...
  async loadDashboardData() {
    ...
    if (stats) {
      if (this.metricTotalUsers)
        this.metricTotalUsers.textContent = (
          stats.total_users || 0
        ).toLocaleString();
      if (this.metricUsersWithPhone)
        this.metricUsersWithPhone.textContent = (
          stats.users_with_phone || 0
        ).toLocaleString();
      if (this.metricPhoneRate) {
        const rate =
          stats.total_users > 0
            ? ((stats.users_with_phone / stats.total_users) * 100).toFixed(1)
            : 0;
        this.metricPhoneRate.textContent = `${rate}%`;
      }
      if (this.metricTotalJobs)
        this.metricTotalJobs.textContent = (
          stats.total_jobs || 0
        ).toLocaleString();
      if (this.metricActiveSessions)
        this.metricActiveSessions.textContent = `${stats.available_sessions || 0}/${stats.total_sessions || 0}`;
      if (this.metricActiveProxies)
        this.metricActiveProxies.textContent = `${stats.active_proxies || 0}/${stats.total_proxies || 0}`;
    }
    ...
    if (jobsData && jobsData.items && this.tableRecentJobs) {
      if (jobsData.items.length === 0) {
        ...
        this.tableRecentJobs.innerHTML = jobsData.items
          .map(
            (job) => `
          <tr>
    ...
    if (usersData && usersData.items && this.tableRecentLeads) {
      if (usersData.items.length === 0) {
        ...
        this.tableRecentLeads.innerHTML = usersData.items
          .map((u) => {
            const fbUrl =
              u.profile_url ||
              (u.facebook_uid
                ? `https://www.facebook.com/${encodeURIComponent(u.facebook_uid)}`
                : u.username
                  ? `https://www.facebook.com/${encodeURIComponent(u.username)}`
                  : "");
            const displayName = this.escapeHtml(
              u.name ||
                u.username ||
                (u.facebook_uid ? `UID: ${u.facebook_uid}` : "Ẩn danh"),
            );
  ...
  async loadJobsData(cursor = null) {
    ...
    this.tableAllJobs.innerHTML = data.items
      .map((job) => {
  ...
  async handleCreateJob(e) {
    ...
    const rawMaxUsers = parseInt(
      document.getElementById("job-opt-max-users")?.value,
      10,
    );
    const maxUsers = isNaN(rawMaxUsers) ? 100 : rawMaxUsers;

    const rawSteps = parseInt(
      document.getElementById("job-opt-steps")?.value,
      10,
    );
    const steps = isNaN(rawSteps) ? 5 : rawSteps;

    const maxDuration =
      parseInt(document.getElementById("job-opt-duration")?.value, 10) || 300;
    const navDelay =
      parseInt(document.getElementById("job-opt-delay")?.value, 10) || 12;
    const depth =
      parseInt(document.getElementById("job-opt-depth")?.value, 10) || 1;
    const enrichProfiles = !!document.getElementById("job-opt-enrich-profiles")
      ?.checked;
    const profileLimit =
      parseInt(document.getElementById("job-opt-profile-limit")?.value, 10) ||
      20;
    const postSteps =
      parseInt(document.getElementById("job-opt-post-steps")?.value, 10) || 0;
    const postDuration =
      parseInt(document.getElementById("job-opt-post-duration")?.value, 10) ||
      30;

    const checkedFields = Array.from(
      document.querySelectorAll('input[name="profile_fields"]:checked'),
    ).map((cb) => cb.value);

    const callFbnumber = document.getElementById("job-opt-call-fbnumber")
      ? !!document.getElementById("job-opt-call-fbnumber").checked
      : true;
    ...
    if (["members", "friends", "followers"].includes(action)) {
      options.max_users =
        maxUsers <= 0 ? 0 : Math.min(Math.max(maxUsers, 1), 1000);
    }

    if (
      action.includes("friends") ||
      action.includes("followers") ||
      action.includes("relationships")
    ) {
      options.depth = depth;
    }
    ...
    const autoBatch =
      action === "members" &&
      !!document.getElementById("job-opt-auto-batch")?.checked;
    const batchCount =
      parseInt(document.getElementById("job-opt-batch-count")?.value, 10) || 5;
    const batchBody = {
      group_url: targets[0],
    ...
    try {
      if (autoBatch && targets.length !== 1) {
        alert(
          "Auto Batch chỉ nhận 1 group URL mỗi lần để tránh trộn checkpoint/progress.",
        );
        return;
      }

      const res = await this.fetchApi(
        autoBatch ? "/api/v1/jobs/group-batches" : "/api/v1/jobs",
        {
          method: "POST",
      ...
      if (autoBatch && res && Array.isArray(res.items)) {
        ...
        this.showToast(
          `Đã tạo ${res.items.length} batch job cho group. Worker sẽ chạy lần lượt.`,
        );
        this.loadJobsData();
      } else if (res && res.id) {
        this.closeModal(this.modalCreateJob);
        this.formCreateJob.reset();
        this.showToast(
          `Đã tạo Crawl Job ${res.id.substring(0, 8)} thành công!`,
        );
        this.loadJobsData();
      } else {
  ...
  async viewJobDetails(jobId) {
    ...
    if (statusEl)
      statusEl.innerHTML = `<span class="pill info">Đang tải...</span>`;
    ...
    if (eventsContainer)
      eventsContainer.textContent =
        "⏳ Đang nạp dòng sự kiện thời gian thực...";
    ...
    try {
      ...
      if (job) {
        ...
        if (targetsEl)
          targetsEl.textContent = `${job.completed_targets || 0}/${job.requested_targets || 0}`;
        if (discoveredEl)
          discoveredEl.textContent = `${job.discovered_users || 0}`;
        if (persistedEl)
          persistedEl.textContent = `${job.persisted_users || 0}`;
      } else {
        if (statusEl)
          statusEl.innerHTML = `<span class="pill danger">Không tìm thấy</span>`;
      }

      if (eventsData && eventsData.items && eventsData.items.length > 0) {
        eventsContainer.innerHTML = eventsData.items
          .map(
            (ev) => `
        ...
        eventsContainer.textContent =
          "Chưa có sự kiện (events) nào được ghi nhận cho Job này.";
      }
    } catch (err) {
      if (eventsContainer)
        eventsContainer.textContent =
          "Lỗi nạp dữ liệu chi tiết: " + err.message;
    }
  }

  async cancelJob(jobId) {
    if (!confirm("Bạn có chắc chắn muốn hủy Job này?")) return;
    const res = await this.fetchApi(`/api/v1/jobs/${jobId}/cancel`, {
      method: "POST",
    });
    if (res) {
  ...
  async retryJob(jobId) {
    const res = await this.fetchApi(`/api/v1/jobs/${jobId}/retry`, {
      method: "POST",
    });
    if (res) {
  ...
  async deleteJob(jobId, isRunning = false) {
    ...
    const res = await this.fetchApi(`/api/v1/jobs/${jobId}`, {
      method: "DELETE",
    });
    if (res) {
  ...
  async loadLeadsData(cursor = null) {
    ...
    this.updateLeadsPaginationUI(
      data.items.length,
      this.leadsPagination.hasMore,
    );

    this.tableAllLeads.innerHTML = data.items
      .map((u) => {
        const p1 = u.phone_1
          ? `<span class="pill success">📞 ${this.escapeHtml(u.phone_1)}</span>`
          : '<span style="color:var(--text-muted);">N/A</span>';
        const p2 = u.phone_2
          ? `<span class="pill info">📞 ${this.escapeHtml(u.phone_2)}</span>`
          : '<span style="color:var(--text-muted);">-</span>';
        const birthDate = u.birth_date
          ? `<span style="font-family:monospace;">${this.escapeHtml(u.birth_date)}</span>`
          : '<span style="color:var(--text-muted);">-</span>';
        const fbUrl =
          u.profile_url ||
          (u.facebook_uid
  ...
  async viewPhoneEvidence(userId) {
    ...
    if (titleEl)
      titleEl.textContent = `Lịch Sử & Bằng Chứng SĐT (User #${userId})`;
    if (tableBody) {
    ...
    try {
      const data = await this.fetchApi(
        `/api/v1/users/${userId}/phone-evidence`,
      );
      if (!data || !data.items || data.items.length === 0) {
      ...
      if (tableBody)
        tableBody.innerHTML = `<tr><td colspan="5" style="text-align:center; color:var(--accent-danger); padding:20px;">Lỗi tải bằng chứng: ${err.message}</td></tr>`;
    }
  }
  ...
  async openEditUserModal(userId) {
    ...
    if (titleEl)
      titleEl.textContent = `Sửa Khách Hàng #${user.id} (${user.name || user.facebook_uid || "Ẩn danh"})`;
  ...
  async deleteUser(userId) {
    if (
      !confirm(
        `Bạn có chắc chắn muốn XÓA vĩnh viễn khách hàng #${userId} và dữ liệu liên quan?`,
      )
    )
      return;
    const res = await this.fetchApi(`/api/v1/users/${userId}`, {
      method: "DELETE",
    });
    if (res) {
  ...
  renderSessionsTable() {
    ...
    this.tableAllSessions.innerHTML = paginated
      .map((s) => {
        const statusClass =
          s.status === "healthy"
            ? "success"
  ...
  async checkSessionLive(sessionName) {
    ...
    try {
      const res = await this.fetchApi(
        `/api/v1/sessions/${encodeURIComponent(sessionName)}/check`,
        {
          method: "POST",
        },
      );
      if (res && res.is_live) {
        ...
        this.showToast(
          `Nick ${sessionName}: ${res?.message || "Không thể xác thực"}`,
        );
      }
  ...
  openEditSessionModal(sessionName) {
    const session = this.sessionsPagination.allItems.find(
  ...
  async handleEditSessionSubmit(e) {
    ...
    const proxy =
      document.getElementById("edit-session-proxy")?.value.trim() || null;
    const status =
      document.getElementById("edit-session-status")?.value || null;

    const res = await this.fetchApi(
      `/api/v1/sessions/${encodeURIComponent(sessionName)}`,
      {
        method: "PATCH",
  ...
  async deleteSession(sessionName) {
    if (
      !confirm(
        `Bạn có chắc chắn muốn XÓA nick '${sessionName}' khỏi Pool và xóa file Cookie?`,
      )
    )
      return;
    const res = await this.fetchApi(
  ...
  async launchSessionBrowser(sessionName) {
    ...
    try {
      const res = await this.fetchApi(
        `/api/v1/sessions/${encodeURIComponent(sessionName)}/launch-login`,
        {
          method: "POST",
        },
      );
      if (res && (res.status === "accepted" || res.status === "success")) {
        this.showToast(
          `✅ Đã mở browser local cho ${sessionName}. Chỉ nhập mật khẩu trong cửa sổ Facebook.`,
        );
      } else {
  ...
  updateSessionsPaginationUI(totalCount, currentPage, totalPages) {
    ...
    if (info)
      info.textContent = `Trang ${currentPage} / ${totalPages} (${totalCount} sessions)`;
  ...
  async handleExtractSession(e) {
    ...
    try {
      const res = await this.fetchApi(
        `/api/v1/sessions/${encodeURIComponent(name)}/launch-login`,
        {
          method: "POST",
      ...
      if (res && res.name) {
        ...
        this.showToast(
          `Đã mở browser local cho session ${res.name}. Nhập tài khoản trực tiếp trong cửa sổ Facebook.`,
        );
        this.loadSessionsData();
      } else {
        alert(
          "Không thể mở browser local. Kiểm tra API có đang chạy trên localhost không.",
        );
      }
    } catch (err) {
  ...
  renderProxiesTable() {
    ...
    this.tableAllProxies.innerHTML = paginated
      .map((p) => {
        const statusClass =
          p.status === "active"
            ? "success"
  ...
  openEditProxyModal(encodedUrl) {
    ...
    const proxy = this.proxiesPagination.allItems.find(
      (p) => p.display_url === rawUrl,
    );
    if (!proxy) {
  ...
  async handleEditProxySubmit(e) {
    ...
    const newUrl =
      document.getElementById("edit-proxy-url")?.value.trim() || null;
  ...
  async deleteProxy(encodedUrl) {
    const rawUrl = decodeURIComponent(encodedUrl);
    if (!confirm(`Bạn có chắc chắn muốn XÓA proxy '${rawUrl}' khỏi ProxyPool?`))
      return;
    const res = await this.fetchApi("/api/v1/proxies", {
      method: "DELETE",
  ...
  updateProxiesPaginationUI(totalCount, currentPage, totalPages) {
    ...
    if (info)
      info.textContent = `Trang ${currentPage} / ${totalPages} (${totalCount} proxies)`;
    if (btnPrev) btnPrev.disabled = currentPage <= 1;
    if (btnNext) btnNext.disabled = currentPage >= totalPages;
  }
  ...
  async handleAddProxies(e) {
    ...
    if (res) {
      ...
      this.showToast(
        `Đã thêm ${proxies.length} proxy vào ProxyPool thành công!`,
      );
      this.loadProxiesData();
    }
  }
  ...
  async loadSettingsData() {
    ...
    if (fbData) {
      ...
      if (tokenInput) {
        ...
        tokenInput.placeholder = data["api_token_configured"]
          ? "Đang dùng token từ .env - để trống vẫn giữ token này"
          : "Bearer Token...";
      }
      ...
      const tokenDetail = document.getElementById(
        "fbnumber-token-expiry-detail",
      );
      ...
      if (tokenBadge && tokenDetail) {
        if (!data.api_token_configured) {
          ...
          tokenDetail.textContent =
            "Chưa có Token FBNumber trong hệ thống. Vui lòng đăng nhập hoặc dán Token để kích hoạt.";
          ...
          data.token_days_remaining !== null &&
          data.token_days_remaining !== undefined
        ) {
          ...
          const expDateStr = data.token_expires_at
            ? new Date(data.token_expires_at).toLocaleString("vi-VN")
            : "";
          ...
          tokenDetail.textContent =
            "Token tùy chỉnh không theo định dạng JWT thông thường.";
        }
      }
    }
    ...
    if (workerData) {
      ...
      const rateLimitInput = document.getElementById(
        "cfg-worker-ratelimit-cooldown",
      );
      ...
      if (cooldownInput)
        cooldownInput.value = workerData.cooldown_seconds ?? 3600;
      if (navDelayInput)
        navDelayInput.value = workerData.navigation_delay_seconds ?? 8;
      if (timeoutInput)
        timeoutInput.value = workerData.job_timeout_seconds ?? 1800;
      if (rateLimitInput)
        rateLimitInput.value = workerData.rate_limit_cooldown_seconds ?? 21600;
      ...
      if (infoEl) {
        if (
          workerData.account_status === "cooldown" &&
          workerData.cooldown_until
        ) {
          ...
          infoEl.textContent =
            "Tài khoản ở trạng thái sẵn sàng thực hiện Job tiếp theo ngay khi có trong hàng đợi.";
        } else {
  ...
  async handleSaveWorkerSettings(e) {
    ...
    const payload = {
      cooldown_seconds:
        parseInt(document.getElementById("cfg-worker-cooldown").value, 10) || 0,
      navigation_delay_seconds:
        parseInt(document.getElementById("cfg-worker-nav-delay").value, 10) ||
        8,
      job_timeout_seconds:
        parseInt(document.getElementById("cfg-worker-timeout").value, 10) ||
        1800,
      rate_limit_cooldown_seconds:
        parseInt(
          document.getElementById("cfg-worker-ratelimit-cooldown").value,
          10,
        ) || 21600,
    };
  ...
  async handleResetCooldown() {
    if (
      !confirm(
        "Bạn có chắc chắn muốn BỎ QUA thời gian Cooldown và đưa nick về trạng thái SẴN SÀNG ngay lập tức?",
      )
    ) {
      return;
    }
    ...
    const res = await this.fetchApi("/api/v1/settings/reset-cooldown", {
      method: "POST",
    });
  ...
  async handleSaveFBNumberSettings(e) {
    ...
    const payload = {
      ...
      timeout_seconds:
        parseFloat(document.getElementById("cfg-fbnumber-timeout").value) ||
        15.0,
      max_retries:
        parseInt(document.getElementById("cfg-fbnumber-retries").value, 10) ||
        2,
      default_country_code:
        document.getElementById("cfg-country-code").value.trim() || "84",
    };
  ...
  async handleExecuteSyncScans(preview = false) {
    ...
    const originalSubmitText =
      btnSubmit?.textContent || "🚀 Bắt Đầu Đồng Bộ & Lưu Vào Database";
    const originalPreviewText =
      btnPreview?.textContent || "🔍 Xem Trước Dữ Liệu (Preview)";
        ...
        if (tbody) {
          ...
          if (res.items && res.items.length > 0) {
            res.items.forEach((item) => {
              ...
              const uidOrName = item.name
                ? `<strong>${item.name}</strong><br><span style="color: var(--text-muted); font-size: 0.75rem;">${item.uid || item.username || "-"}</span>`
                : item.uid || "-";
              const phone1 = item.phone_1
                ? `<span class="pill success" style="font-size: 0.75rem;">${item.phone_1}</span>`
                : "-";
              const phone2 = item.phone_2
                ? `<span class="pill info" style="font-size: 0.75rem;">${item.phone_2}</span>`
                : "-";
              const addr = item.address || "-";
              const gender = item.gender || "-";
              const birthday = item.birthday || "-";
              const scanAt = item.scan_at
                ? new Date(item.scan_at).toLocaleString("vi-VN")
                : "-";
            ...
            tbody.innerHTML =
              '<tr><td colspan="7" style="text-align: center; color: var(--text-muted); padding: 16px;">Không tìm thấy bản ghi nào phù hợp.</td></tr>';
          }
        }
  ...
  async fetchProductApi(
    ...
    if (
      ...
      if (refreshed)
        return this.fetchProductApi(endpoint, options, false, allowReauth);
    }
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      const sensitiveAdminAction =
        endpoint.startsWith("/api/v1/admin/accounts/") &&
        ["/suspend", "/sessions", "/devices/"].some((fragment) =>
          endpoint.includes(fragment),
        );
      if (
        response.status === 403 &&
        allowReauth &&
      ...
      throw new Error(
        errorData.message || errorData.detail || `Lỗi HTTP ${response.status}`,
      );
    }
  ...
  setProductAdminAuthState() {
    ...
    if (!isAdmin) this.clearRevealedLicenseKeys();
    if (this.productAdminLoginPanel)
      this.productAdminLoginPanel.hidden = isAdmin;
    if (this.productAdminWorkspace)
      this.productAdminWorkspace.hidden = !isAdmin;
    if (this.btnProductLogout)
      this.btnProductLogout.hidden = !this.productAccount;
  ...
  async handleCreateLicense(event) {
    ...
    const isUnlimitedDuration =
      document.getElementById("unlimited-duration")?.checked;
    const isUnlimitedContacts = document.getElementById(
      "unlimited-contact-limit",
    )?.checked;
    const isUnlimitedDevices = document.getElementById(
    ...
    const limit = isUnlimitedContacts
      ? 2147483647
      : Number(monthlyLimit?.value || 1000);
    const devices = isUnlimitedDevices
      ? 1000000
  ...
  async copyGeneratedLicenseKey() {
    const value =
      document.getElementById("generated-license-key")?.textContent || "";
    if (!value) return;
    try {
      ...
      this.showToast(
        "Không thể sao chép tự động. Hãy chọn và sao chép key.",
        "error",
      );
    }
  }
  ...
  async loadAdminLicenses(reset = true) {
    ...
    try {
      const cursor =
        this.adminLicensesPagination.cursorHistory[
          this.adminLicensesPagination.pageIndex
        ];
      const query = cursor ? `&cursor=${encodeURIComponent(cursor)}` : "";
      const data = await this.fetchProductApi(
        `/api/v1/admin/license-keys?limit=25${query}`,
      );
  ...
  renderAdminLicenseRows(items) {
    ...
    if (!items.length) {
      this.renderAdminEmptyRow(
        this.tableAdminLicenseKeys,
        6,
    ...
    items.forEach((item) => {
      ...
      this.appendAdminLicenseKeyCell(row, item);
      const isLifetime =
        item.duration.unit === "month" && Number(item.duration.value) >= 1200;
      this.appendAdminTextCell(
        row,
        isLifetime
          ? "Không giới hạn (Vĩnh viễn)"
          : `${item.duration.value} ${item.duration.unit === "month" ? "tháng" : "ngày"}`,
      );
      const isUnlimitedContacts =
        Number(item.monthly_contact_limit) >= 1000000000;
      this.appendAdminTextCell(
        row,
        isUnlimitedContacts
          ? "Không giới hạn"
          : Number(item.monthly_contact_limit).toLocaleString("vi-VN"),
      );
      const isUnlimitedDevices = Number(item.max_devices) >= 1000000;
      this.appendAdminTextCell(
        row,
        isUnlimitedDevices ? "Không giới hạn" : item.max_devices,
      );
  ...
  clearRevealedLicenseKeys() {
    this.licenseRevealGeneration = (this.licenseRevealGeneration || 0) + 1;
    this.tableAdminLicenseKeys?.querySelectorAll(".admin-license-key").forEach((cell) => {
      cell.querySelector(".admin-license-key-text").textContent = cell.dataset.maskedKey;
      const button = cell.querySelector("button");
      if (button?.getAttribute("aria-pressed") === "true") button.title = "Hiện license key";
      button?.setAttribute("aria-pressed", "false");
      button?.setAttribute("aria-label", "Hiện license key");
      const slash = cell.querySelector(".license-eye-slash");
      if (slash) slash.style.display = "none";
    });
  }

  appendAdminLicenseKeyCell(row, item) {
    const cell = document.createElement("td");
    cell.className = "admin-license-key";
    cell.dataset.maskedKey = item.masked_key;
    const text = document.createElement("span");
    text.className = "admin-license-key-text";
    text.textContent = item.masked_key;
    const button = this.createAdminButton("", "btn-secondary");
    button.setAttribute("aria-label", "Hiện license key");
    button.setAttribute("aria-pressed", "false");
    const unavailableMessage = item.can_reveal === false
      ? "Key này được tạo trước khi có tính năng lưu mã hóa, nên không thể xem lại. Chỉ key tạo mới sau khi cập nhật API mới xem được."
      : item.can_reveal !== true
        ? "API chưa cập nhật tính năng xem key. Hãy khởi động lại API rồi tải lại trang."
        : null;
    button.title = unavailableMessage || "Hiện license key";
    ...
    eye.setAttribute("d", "M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0");
    const slash = document.createElementNS(svg.namespaceURI, "path");
    slash.setAttribute("d", "M3 3 21 21");
    slash.setAttribute("class", "license-eye-slash");
    slash.style.display = "none";
    svg.append(eye, slash);
    button.appendChild(svg);
    button.addEventListener("click", async () => {
      if (unavailableMessage) {
        this.showToast(unavailableMessage, "info");
        return;
      }
      if (button.getAttribute("aria-pressed") === "true") {
        ...
        button.setAttribute("aria-pressed", "false");
        button.setAttribute("aria-label", "Hiện license key");
        button.title = "Hiện license key";
        slash.style.display = "none";
        return;
      }
      const generation = this.licenseRevealGeneration;
      button.disabled = true;
      try {
        const result = await this.fetchProductApi(
          `/api/v1/admin/license-keys/${item.id}/reveal`, { method: "POST" },
        );
        if (generation !== this.licenseRevealGeneration || !cell.isConnected) return;
        text.textContent = result.key;
        button.setAttribute("aria-pressed", "true");
        button.setAttribute("aria-label", "Ẩn license key");
        button.title = "Ẩn license key";
        slash.style.display = "";
      } catch (error) {
        if (generation === this.licenseRevealGeneration)
          this.showToast(`Không thể xem license: ${error.message}`, "error");
      } finally {
  ...
  async revokeAdminLicense(item) {
    if (
      !window.confirm(
        `Thu hồi license ${item.masked_key}? Tài khoản đang dùng key này sẽ mất quyền gói trả phí.`,
      )
    )
      return;
    try {
      await this.fetchProductApi(`/api/v1/admin/license-keys/${item.id}`, {
        method: "DELETE",
      });
  ...
  async loadAdminAccounts(reset = true) {
    ...
    try {
      const cursor =
        this.adminAccountsPagination.cursorHistory[
          this.adminAccountsPagination.pageIndex
        ];
      const query = cursor ? `&cursor=${encodeURIComponent(cursor)}` : "";
      const data = await this.fetchProductApi(
        `/api/v1/admin/accounts?limit=25${query}`,
      );
  ...
  renderAdminAccountRows(items) {
    ...
    items.forEach((account) => {
      ...
      this.appendAdminTextCell(
        row,
        account.email_verified_at ? "Đã xác minh" : "Chưa xác minh",
      );
      this.appendAdminTextCell(row, this.formatAdminDate(account.created_at));
      const actions = document.createElement("td");
      actions.className = "admin-row-actions";
      const subscriptionsButton = this.createAdminButton(
        "License",
        "btn-secondary",
      );
      subscriptionsButton.addEventListener("click", () =>
        this.openAdminSubscriptions(account),
      );
  ...
  async suspendAdminAccount(account) {
    if (
      !window.confirm(
        `Tạm khóa tài khoản ${account.email} và đăng xuất toàn bộ phiên?`,
      )
    )
      return;
    try {
      await this.fetchProductApi(
        `/api/v1/admin/accounts/${account.id}/suspend`,
        { method: "POST" },
      );
  ...
  async revokeAdminSessions(account) {
    if (!window.confirm(`Đăng xuất toàn bộ thiết bị của ${account.email}?`))
      return;
    try {
      await this.fetchProductApi(
        `/api/v1/admin/accounts/${account.id}/sessions`,
        { method: "DELETE" },
      );
  ...
  async openAdminSubscriptions(account) {
    ...
    try {
      const data = await this.fetchProductApi(
        `/api/v1/admin/accounts/${account.id}/subscriptions`,
      );
  ...
  renderAdminSubscriptionRows(account, items) {
    ...
    if (!items.length) {
      this.renderAdminEmptyRow(
        this.tableAdminSubscriptions,
        5,
        "Tài khoản chưa gắn license.",
      );
      return;
    }
    items.forEach((item) => {
      ...
      this.appendAdminTextCell(
        ...
        isLifetime
          ? "Không giới hạn (Vĩnh viễn)"
          : this.formatAdminDate(item.ends_at),
      );
      this.appendAdminStatusCell(row, item.status);
      const actions = document.createElement("td");
      if (item.status === "valid" && new Date(item.starts_at) > new Date()) {
        const startButton = this.createAdminButton(
          "Bắt đầu ngay",
          "btn-danger",
        );
        startButton.addEventListener("click", () =>
          this.startAdminSubscriptionNow(account, item),
        );
  ...
  async startAdminSubscriptionNow(account, subscription) {
    const warning =
      "Thời gian còn lại của gói hiện tại sẽ bị mất. Bắt đầu gói đã chọn ngay bây giờ?";
  ...
  async openAdminDevices(account) {
    ...
    try {
      const data = await this.fetchProductApi(
        `/api/v1/admin/accounts/${account.id}/devices`,
      );
  ...
  renderAdminDeviceRows(account, items) {
    ...
    if (!items.length) {
      this.renderAdminEmptyRow(
        this.tableAdminDevices,
        5,
        "Tài khoản chưa có thiết bị.",
      );
      return;
    }
    items.forEach((device) => {
      ...
      if (device.status === "active" && !device.current) {
        const revokeButton = this.createAdminButton("Thu hồi", "btn-danger");
        revokeButton.addEventListener("click", () =>
          this.revokeAdminDevice(account, device),
        );
        actions.appendChild(revokeButton);
      }
      row.appendChild(actions);
  ...
  async revokeAdminDevice(account, device) {
    if (
      !window.confirm(
        `Thu hồi thiết bị ${device.display_name} của ${account.email}?`,
      )
    )
      return;
    try {
  ...
  promptAdminReauthentication(retryAction) {
    if (this.pendingAdminAction) {
      return Promise.reject(
        new Error("Password reauthentication is already pending."),
      );
    }
  ...
  async loadAdminAuditEvents(reset = true) {
    ...
    try {
      const cursor =
        this.adminAuditPagination.cursorHistory[
          this.adminAuditPagination.pageIndex
        ];
      const query = cursor ? `&cursor=${encodeURIComponent(cursor)}` : "";
      const data = await this.fetchProductApi(
        `/api/v1/admin/audit-events?limit=50${query}`,
      );
  ...
  renderAdminAuditRows(items) {
    ...
    if (!items.length) {
      this.renderAdminEmptyRow(
        this.tableAdminAuditEvents,
        5,
        "Chưa có sự kiện quản trị.",
      );
      return;
    }
    items.forEach((event) => {
  ...
  closeModal(modal) {
    ...
    if (el === this.modalAdminReauth) {
      ...
      if (passwordInput) passwordInput.value = "";
      if (this.pendingAdminAction) {
        this.pendingAdminAction.reject(
          new Error("Password reauthentication was cancelled."),
        );
        this.pendingAdminAction = null;
      }
    }
